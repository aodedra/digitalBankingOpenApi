...
class CustomerControllerTest {
    ...
    void retrieveUserByUserName() {
        ...
        Mockito.when(validationService.validateUser(userName)).thenReturn(customer);
        ...
        Mockito.verify(validationService).validateUser(userName);
        Assertions.assertThat(objectResponseEntity).usingRecursiveComparison().isEqualTo(body);
        Mockito.verify(validationService).validateImages(Mockito.any());
